...
- **"Continuação do ANEXO X"**: quando um anexo ocupa mais de uma página — o **ANEXO I** (sumário + corpo normativo) e os anexos de imagem —, da **segunda página em diante** aparece no topo, centralizado, "*Continuação do ANEXO I*" (o número do próprio anexo); na primeira página e em anexos de uma página só não aparece nada. Implementado por um `page-sequence-master` (`a4-anexo`: 1ª página `a4`, demais `a4-continuacao`, com região de cabeçalho própria) e coberto por `DocumentoFoBuilderTest`, que renderiza o FO no FOP e conta as páginas. Só existe no PDF (o HTML e a prévia não têm paginação);
