...
- enquanto a alteração não é publicada, a cláusula usa o placeholder `XYZ/ABC` (como toda cláusula pendente); ao publicar, `EmendaService.consolidarPublicacao` congela a cláusula em `ItemAnexoParteNormativa.clausulaRenumeracao` (coluna `tx_clausula_renumeracao`), e o riscado passa a ser **permanente**;
...
O cabeçalho da NPA é uma tabela de campos, não uma parte preliminar. Os campos que só ela tem ficam em `t_documento_npa` (`CamposDaNpa`, uma linha por documento), fora de `Documento`:
