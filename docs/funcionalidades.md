...
| **Publicados e revogados** | `PUBLICADO` e `REVOGADO` **sem etapa em andamento** (`SEM_ETAPA`), **de todas as OMs**, mais recentes primeiro | *Visualizar* |

**Cada documento fica num só lugar do hub.** Um documento publicado que está sendo alterado (ou em revogação) tem duas situações ao mesmo tempo — a oficial ("Publicado": a versão em vigor continua valendo) e a etapa interna (a alteração) —, e por isso aparece nos cards de **trabalho** (em andamento, e aguardando minha ação se for o caso), com **os dois selos** ("Publicado" e a etapa; `selosDaLinha`), e **não** no de publicados e revogados, para onde volta quando a etapa termina. Um documento nunca publicado leva só o selo da etapa. O card de publicados tem um tooltip (ⓘ) que explica esse critério.
