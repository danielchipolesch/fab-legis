...
A `BuscaPage` (`/busca`, aberta pelo botão **"Busca no conteúdo"** no card de filtros da `HomePage`, ao lado de "Limpar" e dos botões de lista/cards — sem papel específico, mesma regra de visualização universal do acervo; ela não fica mais no menu do usuário) procura **dentro do texto** dos dispositivos de **todos os documentos, de qualquer aba** (o campo da homepage filtra só a aba atual, por assunto ou número). O botão leva o **termo já digitado** no campo da homepage: a busca abre com ele preenchido (`?q=`) e já executada. Os dispositivos são Artigo, Parágrafo, Inciso, Cláusula..., não só metadado do documento como na busca da `HomePage`. Cada resultado é um dispositivo específico, com o trecho onde o termo foi encontrado já destacado, e um clique abre o documento correspondente na tela de visualização.
