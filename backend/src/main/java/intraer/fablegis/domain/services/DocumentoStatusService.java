...
import intraer.fablegis.domain.regras.AcaoDeEtapa;
...
public class DocumentoStatusService {
    ...
    public DocumentoResponseSemAnexoTextualDto changeStatus(Long id, DocumentoStatusRequestDto request) throws RuntimeException {
        ...
        if (acao == AcaoDeEtapa.PUBLICAR || acao == AcaoDeEtapa.REVOGAR) {
            // O que se registra na publicação (portaria + BCA num ato normativo; Boletim Interno numa NPA) é
            // regra da espécie.
            regras.para(documento.getEspecieNormativa()).registroDaPublicacao()
                    .registrar(documento, request, acao, bcaAnterior);
        }
