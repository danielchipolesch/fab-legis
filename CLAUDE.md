...
## Regras que variam por espécie entram por interface do regime

Espécies normativas obedecem a **regimes** (`RegimeNormativo`: hoje `ATO_NORMATIVO`; a NPA será o próximo). Um comportamento que varia por espécie — numeração, estrutura inicial, layout de PDF/HTML, rótulo dos anexos, ciclo de vida, campos do cabeçalho — **entra como método de uma interface do pacote `domain.regimes`** (ou de uma interface nova ali), implementada por cada regime e obtida por `RegimesNormativos.para(especie)`. **Nunca** teste a sigla ou o nome da espécie num serviço (`if (especie == NPA)`). Ver [`docs/arquitetura.md`](docs/arquitetura.md#regimes-normativos-regras-por-especie-atras-de-interfaces).
