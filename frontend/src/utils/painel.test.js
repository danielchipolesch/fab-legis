...
describe('acaoDaLinha — meus documentos em andamento', () => {
  it.each(['RASCUNHO', 'MINUTA', 'EM_ALTERACAO'])('%s: Editar, quando sou autor ou coautor', (etapa) => {
    const a = acaoDaLinha('em_andamento', doc({ situacao_local: etapa }))
  ...
  it('sem posse (nem autor nem coautor) não edita', () => {
    expect(acaoDaLinha('em_andamento', doc({ eh_autor_ou_coautor: false })).rotulo).toBe('Acompanhar')
  })
})
...
describe('detalhesDoDocumento', () => {
  ...
  it('em andamento: com quem o documento está', () => {
    expect(valor(detalhesDoDocumento('em_andamento', doc({ situacao_local: 'MINUTA' })), 'Com')).toBe('Autor e coautores')
    expect(valor(detalhesDoDocumento('em_andamento', doc({ situacao_local: 'EM_REVISAO', revisor_atribuido_nome: 'Ana' })), 'Com')).toBe('Ana')
    expect(valor(detalhesDoDocumento('em_andamento', doc({ situacao_local: 'EM_PUBLICACAO', publicador_atribuido_nome: 'Rui' })), 'Com')).toBe('Rui')
  })
